...
class BN_Input:
    ...
    def video_batch_n_ref_no_label(self, idx0, idx1, batch=5):
        ...
        vector_tensor = np.zeros(shape=(batch, n_in_frames *(self.vector_dim + 1)), dtype=np.float32)
        ...
        for i in range(batch):
            ...
            idx_np[:] = idx
            vector_diff = np.concatenate((self.vectors[idx].flatten(),idx_np/self.n_frames))
