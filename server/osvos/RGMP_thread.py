...
# 导入自定义的库
root_folder8 = os.path.dirname(os.path.realpath(__file__))#当前py所在目录
parent_folder8 = os.path.dirname(root_folder8)
sys.path.append(parent_folder8)
sys.path.append(os.path.join(parent_folder8,'osvos'))
from RGMP_model import RGMP
from RGMP_utils import ToCudaVariable, ToLabel, DAVIS, upsample, downsample


class VosRGMP_thread():#执行交互式图像分割的ivs子线程类
