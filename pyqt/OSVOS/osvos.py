...
sys.path.append(os.path.join(parent_folder3,'OSVOS'))
...
class finetuneThread(QThread):
    finishSin=QtCore.pyqtSignal(str)#信号,int是信号承载数据的类型

    def __init__(self,osvosinfo):
        self.osvosinfo=osvosinfo
        QThread.__init__(self)

    def run(self):#通过.Start()调用此run()函数
        path_prefix=self.osvosinfo['path_prefix']
        model_folder=self.osvosinfo['model_folder']
        mask_img=self.osvosinfo['mask_img']
        mask_id=self.osvosinfo['mask_id']
        frames_path=self.osvosinfo['frames_path']
        result_path=self.osvosinfo['result_path']
        max_training_iters=self.osvosinfo['itertime']
        #在执行耗时操作的地方添加,使得一边执行耗时程序一边刷新界面，让人感觉界面很流畅
        QApplication.processEvents()
        seq_name=path_prefix.split('/')[-1]
        gpu_id=0
        #result_path=os.path.join(path_prefix,'Results')
        parent_path=os.path.join(model_folder,'OSVOS_parent.ckpt-50000')
        logs_path = os.path.join(path_prefix, 'Models')
        #max_training_iters = self.ft.itertime
        # Define Dataset
        test_frames = sorted(os.listdir(frames_path))
        test_imgs = [os.path.join(frames_path, frame) for frame in test_frames]
        train_imgs = [os.path.join(frames_path,'{:05d}.jpg'.format(mask_id)) + ' ' +
                      mask_img]
        ...
        with tf.Graph().as_default():
            with tf.device('/gpu:' + str(gpu_id)):
                ...
                train_finetune(dataset, parent_path, side_supervision, learning_rate, logs_path,
                                     max_training_iters,
                                     save_step, display_step, global_step, iter_mean_grad=1, ckpt_name=seq_name)
        QApplication.processEvents()
        with tf.Graph().as_default():
            with tf.device('/gpu:' + str(gpu_id)):
                checkpoint_path = os.path.join(path_prefix, 'Models', seq_name + '.ckpt-' + str(max_training_iters))
                test(dataset, checkpoint_path, result_path)
        QApplication.processEvents()
        self.finishSin.emit(result_path)
...
def _train(dataset, initial_ckpt, supervison, learning_rate, logs_path, max_training_iters, save_step, display_step,
    ...
    Returns:
    """
    model_name = os.path.join(logs_path, ckpt_name + ".ckpt")#存储的训练好的模型
    if config is None:#gpu，cpu配置
    ...
    with tf.Session(config=config) as sess:
        ...
        if (step - 1) % save_step != 0:
            ...
            print("Model saved in file: %s" % save_path)
        time2 = datetime.now()
        print('Finetune training cost time:{}'.format(time2-time1))
