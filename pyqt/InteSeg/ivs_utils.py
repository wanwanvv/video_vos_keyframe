...
def ToCudaPN(mask):
    ...
    P = torch.unsqueeze(torch.from_numpy(P), dim=0).float() #对数据维度进行扩充
...
def Dilate_mask(mask, num_objects):
    ...
    new_mask = -np.ones_like(mask, dtype=np.int64) #返回一个用1填充的和输入数组形状相同的数组
    for o in range(num_objects+1): # include bg scribbles
        bmask = (mask == o).astype(np.uint8) #等于0值的位置赋为1,不等于的地方赋为）
...
def load_frames(fnames, size=None, num_frames=None):
    frame_list = []
    for i, fname in enumerate(fnames):
        ...
        if num_frames and i > num_frames:
            break
    frames = np.stack(frame_list, axis=0) #和np.concat类似
    return frames
